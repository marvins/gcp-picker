...
class Sobel_Edge_Settings:
    ...
    Attributes:
        ...
        threshold:   Optional threshold for edge magnitude (0 = auto/Otsu).
    """
    kernel_size: int = 3
    dilation:    int = 3
    threshold:   int = 0
...
class Sobel_Edges:
    ...
    def detect(self, image: np.ndarray) -> np.ndarray:
        ...
        Returns:
            Edge magnitude image (H, W) uint8, normalized to [0, 255].
        """
        gray = self._to_grayscale(image)
        gray = self._normalize(gray)

        sobel_x = cv2.Sobel(gray, cv2.CV_64F, 1, 0, ksize=self._settings.kernel_size)
        sobel_y = cv2.Sobel(gray, cv2.CV_64F, 0, 1, ksize=self._settings.kernel_size)
        ...
        if self._settings.threshold > 0:
            _, edges = cv2.threshold(edges, self._settings.threshold, 255, cv2.THRESH_BINARY)
        elif self._settings.threshold == 0:
