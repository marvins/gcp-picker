...
class Auto_Matcher:
    """Full automatic GCP matching pipeline.
    ...
    Usage::
        ...
        result = Auto_Matcher(settings).run(test_image, ref_chip, geo_transform)
        if result.success:
    ...
    def run(self,
        ...
        """Execute the full pipeline and return a ``Match_Result``.
        ...
        result.candidate_pixels = cand_pixels
        result.candidate_geos   = cand_geos
        result.inlier_mask      = np.ones(len(cand_pixels), dtype=bool)
        result.elapsed_sec      = time.perf_counter() - t0

        logging.info(
            f'Auto_Matcher: done — {len(cand_pixels)} candidate GCPs '
            f'in {result.elapsed_sec:.2f}s'
        )
        return result
